...
export const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  }, [currentUser, isOpen, isCheckingOut]);
  ...
  const handleGenerateInvoice = async (e: React.FormEvent) => {
    ...
    if (!currentUser) {
      setErrorMessage('Debes registrarte o iniciar sesión antes de realizar tu compra.');
      onOpenAuth('register');
      return;
    }
    ...
    const finalDoc = (currentUser.document || userDocInput).trim();
    const finalPhone = (currentUser.phone || userPhoneInput).trim();
    ...
    if (!finalAddress || finalAddress.length < 5) {
      setErrorMessage('Por favor ingresa tu dirección completa de entrega para tu pedido.');
      return;
    }
    ...
    try {
      const newInvoice = await InvoiceController.createInvoice({
        ...
        paymentMethod,
        notes: notes.trim() || 'Factura cliente registrado',
        currentUser,
      });
          ...
          <div className="flex-1 overflow-y-auto p-3.5 sm:p-6 space-y-4 sm:space-y-6">
            ...
            {cart.length === 0 ? (
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                {!currentUser && (
                  <div className="p-4 rounded-3xl bg-amber-500/10 dark:bg-amber-950/30 border-2 border-amber-300 dark:border-amber-800/60 space-y-3 shadow-xs">
                    <div className="flex items-start gap-3">
                      <div className="w-10 h-10 rounded-2xl bg-orange-500 text-white flex items-center justify-center shrink-0 shadow-xs mt-0.5">
                        <Lock className="w-5 h-5" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <h4 className="text-xs sm:text-sm font-black text-slate-900 dark:text-white font-display">
                            Registro Requerido para Comprar
                          </h4>
                          <span className="text-[10px] font-black uppercase text-amber-800 dark:text-amber-300 bg-amber-200/80 dark:bg-amber-900/60 px-2 py-0.5 rounded-full">
                            Obligatorio
                          </span>
                        </div>
                        <p className="text-[11px] sm:text-xs text-slate-600 dark:text-slate-300 mt-1 leading-relaxed">
                          Puedes añadir y gestionar todos los productos en tu carrito, pero <strong>antes de comprar debes registrarte o iniciar sesión</strong> para emitir tu factura fiscal legal en COP.
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 pt-0.5">
                      <button
                        ...
                        className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-orange-500 hover:bg-orange-600 text-white text-xs font-black shadow-xs transition-all cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl border border-yellow-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-xs font-bold hover:bg-yellow-50 dark:hover:bg-slate-700 transition-all cursor-pointer"
                      >
              ...
              /* Checkout / Invoice Billing Form (Only for authenticated users) */
                ...
                !currentUser ? (
                  <div className="space-y-2">
                    <button
                      ...
                      type="button"
                      onClick={() => onOpenAuth('register')}
                      className="w-full py-3 sm:py-3.5 rounded-2xl bg-orange-500 hover:bg-orange-600 text-white text-xs sm:text-sm font-black shadow-[0_4px_0_0_rgba(234,88,12,1)] active:translate-y-1 active:shadow-none transition-all flex items-center justify-center gap-2 cursor-pointer"
                    >
